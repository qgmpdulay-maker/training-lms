<x-app-layout>
    <div class="py-10"
        x-data="{
            ...
            competencies: {{ Js::from(collect($facets)->map(fn ($f) => ['facet' => $f, 'competency' => null, 'importance' => null])->values()->all()) }},
            ...
            async submit() {
                ...
                try {
                    const res = await fetch('{{ route('training-needs-assessment.academe.store') }}', {
                        method: 'POST',
                        headers: {
    ...
    >
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mb-8">{{ __('TNA Tool for Academic Institutions — answer honestly so we can identify the training your institution needs most.') }}</p>
            ...
            <div x-show="step === 'form'" class="space-y-8">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-6 sm:p-8">
                    ...
                    <p class="text-xs text-gray-400 dark:text-gray-500 mb-6">{{ __('Competency: 1 = Basic … 5 = Excellent.  Importance: 1 = Not Important … 5 = Very Important.') }}</p>

                    <div class="space-y-6">
                        <template x-for="(item, index) in competencies" :key="index">
                            <div class="border border-gray-200 dark:border-gray-600 rounded-lg p-4">
                                <p class="text-sm font-medium text-gray-700 dark:text-gray-200 mb-3" x-text="item.facet"></p>
                                    ...
                                    <div>
                                        ...
                                        <div class="flex gap-1.5">
                                            <template x-for="n in 5" :key="'c'+n">
                                                <button type="button" @click="item.competency = n"
                                                    :class="item.competency === n ? 'bg-[#152A4E] text-white border-[#152A4E]' : 'border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-400 hover:border-[#152A4E]'"
                                                    class="w-9 h-9 rounded-lg border-2 text-sm font-semibold transition" x-text="n"></button>
                                            </template>
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div class="flex gap-1.5">
                                            <template x-for="n in 5" :key="'i'+n">
                                                <button type="button" @click="item.importance = n"
                                                    :class="item.importance === n ? 'bg-[#E2762D] text-white border-[#E2762D]' : 'border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-400 hover:border-[#E2762D]'"
                                                    class="w-9 h-9 rounded-lg border-2 text-sm font-semibold transition" x-text="n"></button>
                                            </template>
                                        </div>
                                    </div>
            ...
            <div x-show="step === 'results'" class="space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-6 sm:p-8">
                    ...
                    <div class="space-y-2">
                        <template x-for="(c, index) in results" :key="index">
