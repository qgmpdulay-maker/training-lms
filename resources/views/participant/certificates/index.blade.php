        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if ($certificates->isEmpty())
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-10 text-center">
                    <p class="text-gray-500 dark:text-gray-400">{{ __("No certificates on file yet — they'll show up here automatically once a training you attended is marked Completed.") }}</p>
                </div>
            @else
                <div class="space-y-4">
                    @foreach ($certificates as $certificate)
                        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-5 sm:p-6">
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                <div>
                                    <h2 class="font-bold text-[#152A4E] dark:text-white">{{ $certificate->trainingRequest->training_title }}</h2>
                                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                        {{ $certificate->issued_on->format('F j, Y') }}
                                        @if ($certificate->typeLabel())
                                            &middot; {{ $certificate->typeLabel() }}
                                        @endif
                                    </p>
                                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">{{ __('Certificate Code') }}: {{ $certificate->code }}</p>
                                </div>
