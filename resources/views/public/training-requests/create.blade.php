...
<x-public-layout>
    <div class="pt-28 sm:pt-32 pb-16">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <h1 class="text-2xl sm:text-3xl font-bold text-[#152A4E] mb-1">{{ __('Request for Training') }}</h1>
            <p class="text-sm text-gray-500 mb-2">{{ __("For LGUs and National Government Agencies requesting a training from OCD. Fill this out one step at a time — you can go back at any point before you submit.") }}</p>
            <p class="text-sm text-gray-500 mb-8">
                <span class="font-semibold text-[#152A4E]">{{ __('No account needed.') }}</span>
                {{ __('The request is filed by your agency or LGU, not by an individual participant — you only need the contact details of the person OCD should coordinate with.') }}
            </p>

            <div
                x-data="{
                    step: {{ $initialStep }},
                    agencyType: '{{ old('agency_type', '') }}',
                ...
                <form method="POST" action="{{ route('public.training-requests.store') }}" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div id="step-2" x-show="step === 2" x-cloak>
                        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 sm:p-8 space-y-5">
                            ...
                            <div>
                                <label for="lgu" class="block text-sm font-medium text-gray-700 mb-1.5">
                                    <span x-show="agencyType === '{{ \App\Models\TrainingRequest::AGENCY_TYPE_LGU }}'" x-cloak>{{ __('City / Municipality / Barangay') }}</span>
                                    <span x-show="agencyType !== '{{ \App\Models\TrainingRequest::AGENCY_TYPE_LGU }}'">{{ __('LGU (city/municipality/barangay, if applicable)') }}</span>
                                </label>
                                <input id="lgu" type="text" name="lgu"
                                    :required="agencyType === '{{ \App\Models\TrainingRequest::AGENCY_TYPE_LGU }}'"
                                    value="{{ old('lgu') }}"
                                    class="w-full rounded-lg border-gray-300 focus:border-[#152A4E] focus:ring-[#152A4E] text-base py-3">
                                <p class="text-xs text-gray-500 mt-1" x-show="agencyType === '{{ \App\Models\TrainingRequest::AGENCY_TYPE_LGU }}'" x-cloak>
                                    {{ __('Which LGU this request is for — this is what OCD uses to scope the training.') }}
                                </p>
