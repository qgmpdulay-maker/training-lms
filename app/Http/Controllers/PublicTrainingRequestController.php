...
/**
 * Public, unauthenticated Technical Assistance request form.
 *
 * The requester here is the Agency/LGU itself — not a Regional Admin filing
 * on its behalf, and not an individual participant. There is no account and
 * no login: the agency identifies itself by agency type, name, LGU, and a
 * contact person, so `user_id` stays null on the resulting record (see
 * 2026_09_08_055131_make_user_id_nullable_on_training_requests_table.php).
 *
 * Every request lands in STATUS_SUBMITTED tagged SOURCE_PUBLIC_PORTAL and
 * enters the same review pipeline OCD Regional Offices already manage from
 * the admin Summary page — Super Admin's Schedule Training flow
 * (Admin\SuperAdmin\TrainingController) stays as it is for trainings OCD
 * schedules itself.
 */
class PublicTrainingRequestController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            // An LGU request has to name the city/municipality/barangay it's
            // for — that's what scopes it. An NGA request has no LGU.
            'lgu' => ['required_if:agency_type,'.TrainingRequest::AGENCY_TYPE_LGU, 'nullable', 'string', 'max:255'],
            ...
            'lgu.required_if' => 'Please name the city, municipality, or barangay this request is for.',
            'tna_completed.accepted' => 'Please confirm the Training Needs Assessment has been completed.',
            'logistics_acknowledged.accepted' => 'Please confirm the requesting agency will arrange the items listed above.',
        ]);
        ...
        // Filed by the agency itself — no account, so nobody owns this record
        // until an admin picks it up in Summary.
        $trainingRequest->user_id = null;
        $trainingRequest->status = TrainingRequest::STATUS_SUBMITTED;
        ...
        // Both attachments carry an agency's own paperwork, so they go on the
        // private disk (storage/app/private) rather than public/storage where
        // anyone guessing the filename could read them. Admins open them
        // through Admin\SummaryController::attachment().
        if ($request->hasFile('tna_file')) {
            $trainingRequest->tna_file_path = $request->file('tna_file')->store('training-requests/tna', 'local');
        }

        if ($request->hasFile('signed_letter')) {
            $trainingRequest->signed_letter_path = $request->file('signed_letter')->store('training-requests/letters', 'local');
        }
